...
class ImportDocument(Base):
    ...
    # Campos de OCR
    extracted_text = Column(Text)  # Texto extraído pelo OCR
    ocr_confidence = Column(Float)  # Confiança do OCR (0-1)
    ocr_method = Column(String)  # Método usado (easyocr, tesseract)
    classification_confidence = Column(Float)  # Confiança da classificação
